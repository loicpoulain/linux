			      ...
			      struct v4l2_isp_params_block_header *hdr)
{
	...
	/* if config and context has not changed, nothing to do */
	if ((ctx == ope->hw_ctx) && !dirty)
		return false;
	...
	dev_dbg(ope->dev, "IQ module (%u) update (flags:0x%x) \n", hdr->type, hdr->flags);
...
{
	struct camss_params_demo *demo = &ctx->config.demo;
	struct ope_dev *ope = ctx->ope;
...
static void ope_params_apply_demo(void *priv, const union camss_isp_params_block *block)
{
	struct ope_ctx *ctx = priv;
...
static void ope_job_finish(struct ope_ctx *ctx, enum vb2_buffer_state state)
{
	...
	dev_dbg(ctx->ope->dev, "done ctx=%p\n", ctx);
	...
	if (dst)    camss_isp_buf_done(dst,    state);
	if (src)    camss_isp_buf_done(src,    state);
	if (params) camss_isp_buf_done(params, state);
...
static void ope_run_job(void *priv)
{
	struct ope_ctx *ctx = priv;
	struct ope_dev *ope = ctx->ope;
	dma_addr_t src, dst;
	...
	if (!src || !dst) {
		dev_err(ope->dev, "Job cannot run, missing buffer\n");
...
{
	unsigned int idx = (unsigned int)(uintptr_t)video_get_drvdata(video_devdata(file));
...
static int ope_qbuf(struct file *file, void *priv, struct v4l2_buffer *buf)
{
	...
	return vq ? vb2_qbuf(vq, vdev->v4l2_dev->mdev, buf) : -EINVAL;
}
...
static int ope_prepare_buf(struct file *file, void *priv, struct v4l2_buffer *buf)
{
	...
	return vq ? vb2_prepare_buf(vq, vdev->v4l2_dev->mdev, buf) : -EINVAL;
}
...
static int ope_create_bufs(struct file *file, void *priv, struct v4l2_create_buffers *create)
{
	...
	return vq ? vb2_create_bufs(vq, create) : -EINVAL;
}
...
static int ope_expbuf(struct file *file, void *priv, struct v4l2_exportbuffer *eb)
{
	...
	return vq ? vb2_expbuf(vq, eb) : -EINVAL;
}
...
static int ope_streamon(struct file *file, void *priv, enum v4l2_buf_type type)
{
	...
	return vq ? vb2_streamon(vq, type) : -EINVAL;
}
...
static __poll_t ope_poll(struct file *file, poll_table *wait)
{
	...
	for (i = 0; i < OPE_QUEUE_COUNT; i++)
		poll_wait(file, &ctx->vqs[i].done_wq, wait);
	poll_wait(file, &fh->wait, wait);

	for (i = 0; i < OPE_QUEUE_COUNT; i++) {
		struct vb2_queue *vq = &ctx->vqs[i];
		unsigned long flags;
