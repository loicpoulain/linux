...
{
	struct camss_ope_params_wb_gain *wb = &ctx->config.wb_gain;
...
{
	struct camss_ope_params_chroma_enhan *cc = &ctx->config.chroma_enhan;
	struct ope_dev *ope = ctx->ope;
...
{
	struct camss_ope_params_color_correct *cc = &ctx->config.color_correct;
	struct ope_dev *ope = ctx->ope;
...
static const struct ope_config ope_default_config = {
	.wb_gain = {
		.header.type = CAMSS_OPE_PARAMS_WB_GAIN,
	...
	.chroma_enhan = { /* RGB -> YUV values from BT.601/Full-Range */
		.header.type = CAMSS_OPE_PARAMS_CHROMA_ENHAN,
		.header.flags = V4L2_ISP_PARAMS_FL_BLOCK_ENABLE |
				CAMSS_ISP_PARAMS_FL_BLOCK_DIRTY,
		.luma_v0  = 0x04d, /* 0.299 */
		.luma_v1  = 0x096, /* 0.587 */
