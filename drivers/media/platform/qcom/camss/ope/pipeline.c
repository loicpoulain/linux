...
static void isp_pipeline_release(struct kref *ref)
{
	struct camss_isp_pipeline *pipeline =
		container_of(ref, struct camss_isp_pipeline, refcount);

	mutex_destroy(&pipeline->vdev_lock);
	kfree(pipeline);
}

static void isp_vdev_release(struct video_device *vdev)
{
	struct camss_isp_pipeline_entity *slot =
		container_of(vdev, struct camss_isp_pipeline_entity, vdev);

	kref_put(&slot->pipeline->refcount, isp_pipeline_release);
}
...
{
	if (pipeline)
		kref_put(&pipeline->refcount, isp_pipeline_release);
}
