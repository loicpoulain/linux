 ...
 * @registered: set once the entity has been successfully registered, so that
 *              teardown only undoes what actually succeeded.
 * @pipeline: owning pipeline, used by the video device release callback.
 ...
 * @refcount:     Keeps the pipeline allocation alive for as long as any of its
 *                registered video devices is: video_device.lock points into
 *                this structure, so it must outlive open file handles.
